<template>
  <div class="h-screen bg-[#F7F8FA]">
    ...
    <div class="mx-auto py-6 px-5 max-w-7xl">
      ...
      <h3 class="mr-2 py-2 font-bold text-xl underline decoration-4 decoration-[#FFB508] underline-offset-1">لیست اپراتور</h3>

      <!--add product-->
      <div class="py-6">
        <nuxt-link class="py-3 flex justify-center items-center bg-[#4B1752] text-white font-bold rounded hover:bg-[#2C0531]" to="/admin/operators/add">
      ...
      <div class="border border-gray-200 rounded overflow-x-auto min-w-full bg-white">
        ...
        <table class="min-w-full text-sm align-middle whitespace-nowrap">
          ...
          <!-- Table Body -->
          <tbody v-for="(i,n) in getOperators" :key="n">
...
export default {
  ...
  mounted() {
    this.$store.dispatch('ingredients/getOperators')
  },

  computed: {
    getOperators() {
      return this.$store.getters['ingredients/getOperators']
    },
  },
