<template>
  <div class=" bg-[#F7F8FA]">
    ...
    <div class="mx-auto py-6 px-5 max-w-7xl">
      ...
      <div class="container">
        <h3 class="mr-2 py-4 font-bold text-xl underline decoration-4 decoration-[#FFB508] underline-offset-1">ایجاد اپراتور جدید</h3>
...
export default {
  ...
  methods: {
    ...
    save(){
      this.$store.dispatch('ingredients/createOperators', {
        name:this.name,
        // image_id:this.image ? this.image.id :null,
        thumbnail: this.thumbnail,
      })
        .then(res=>{
          this.$router.replace('/admin/operators/list')
        })
        .catch(async err=>{
